...
- Maestro сохраняет детерминированный сценарий на эмуляторе и добавляет обязательный physical-device smoke на Samsung Galaxy S22+ (`SM-S906*`) по wireless ADB. Команда `pnpm test:e2e:android:s22` должна явно выбрать этот телефон, собрать и установить APK с production HTTPS API, очистить состояние приложения, войти с PIN из переменной окружения, выбрать `Thai formal` и мужской пол, перевести `Спасибо`, проверить `ขอบคุณครับ` и наличие обеих транслитераций. Pairing выполняется один раз через `pnpm android:s22:pair -- HOST:PAIR_PORT`; PIN, IP и pairing-код не сохраняются в репозитории. [Expo local Android build](https://docs.expo.dev/guides/local-app-development/), [Expo Maestro E2E](https://docs.expo.dev/eas/workflows/examples/e2e-tests/).
